...
best_C_max, best_stat_df, C_max_list = FJS_GA(func=cal_C_max,machines=machines,times=times,Max_iter=2000,global_init=True,dynamic_change=True,TLBO=True)
C_max_list = pd.DataFrame(C_max_list)
writer = pd.ExcelWriter('Solution_RealCase_global_change_2000.xlsx')#文件的名字也要改下最好把运行次数也加到文件里去
